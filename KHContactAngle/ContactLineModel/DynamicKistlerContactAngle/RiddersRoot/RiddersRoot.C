...
) const
{
    ...
    if ((fl > 0 && fh < 0) || (fl < 0 && fh > 0))
    {
        ...
        for (label nIter = 0; nIter < maxIter; nIter++)
        {
            ...
            {
                // Never get here
                FatalErrorIn("RiddersRoot::root(const scalar, const scalar)")
                    << "Error in search logic. This should not happen."
                    << abort(FatalError);
            }
        ...
        FatalErrorIn("RiddersRoot::root(const scalar, const scalar)")
            << "Maximum number of iterations (" << maxIter << ") exceeded."
            << abort(FatalError);
    }
    ...
    {
        FatalErrorIn("RiddersRoot::root(const scalar x0, const scalar x1)")
            << "Root is not bracketed in the interval [" << x0 << ", " << x1 << "]." << nl
            << "    f(x0) = " << fl << nl
            << "    f(x1) = " << fh
            << abort(FatalError);
    }
